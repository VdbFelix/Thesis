...
corpus = load_data("your_corpus.txt")

#Because corpus is too long, need to chunk the text
import string
nlp = spacy.load("en_core_web_sm") #using spaCy's tokeniser
...
def create_wordvecs(corpus, model_name):
    ...
    w2v_model = Word2Vec(min_count=1,
                        window=2,
                        vector_size=100, #industry standard is to create between 100 to 300 vector dimensions
                        sample=6e-5,
                        alpha=0.03,
                        min_alpha=0.0007,
                        negative=20)
